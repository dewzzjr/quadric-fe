...
ALTER TABLE `reg_data` DROP `ALPRO`, DROP `BEFORE_CA`, DROP `CHANEL`, DROP `CHANEL_APP`, DROP `CSEG`, DROP `Cancel`, DROP `MTD MTTI`, DROP `APP`, DROP `DURASI`, DROP `DURASI_FO`, DROP `DURASI_PI`, DROP `DURASI_RE`, DROP `DURASI_UN`, DROP `DURASI_VA`, DROP `ISISKA_TGLVA`, DROP `JENISPSB`, DROP `KANDATEL`, DROP `KAWASAN`, DROP `KODEFIKASI_SC`, DROP `NDEM`, DROP `Number of Records`, DROP `PS_DONE`, DROP `REASON_CANCEL`, DROP `SC_TGLCA`, DROP `SC_TGLCREATE`, DROP `SC_TGLFO`, DROP `SC_TGLPI`, DROP `SC_TGLUNSC`, DROP `SLG (copy)`, DROP `STATUS`, DROP `STATUS_INDIHOME`, DROP `STATUS_SERVICE`, DROP `TYPE_PIPE`;
